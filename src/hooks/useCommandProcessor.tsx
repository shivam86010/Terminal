...
import { MatrixOutput } from "../Components/Terminal/Outputs/MatrixOutput";
import { NeofetchOutput } from "../Components/Terminal/Outputs/NeofetchOutput";
import { CowsayOutput } from "../Components/Terminal/Outputs/CowsayOutput";
import { AsciiArtOutput } from "../Components/Terminal/Outputs/AsciiArtOutput";
import { FortuneCookieOutput } from "../Components/Terminal/Outputs/FortuneCookieOutput";
import { WeatherOutput } from "../Components/Terminal/Outputs/WeatherOutput";
import { HackerOutput } from "../Components/Terminal/Outputs/HackerOutput";
import { LoadingOutput } from "../Components/Terminal/Outputs/LoadingOutput";
import { TypewriterOutput } from "../Components/Terminal/Outputs/TypewriterOutput";
import { HistoryOutput } from "../Components/Terminal/Outputs/HistoryOutput";
import { GrepOutput } from "../Components/Terminal/Outputs/GrepOutput";
...
export const useCommandProcessor = () => {
  ...
  const processCommand = useCallback(
    (input: string) => {
      ...
      if (trimmedInput === "resume" || trimmedInput === "/resume") {
        addCommand(
          input,
          <div className="font-mono">
            <p className="text-terminal-success">🚀 Opening resume page...</p>
            <p className="text-muted-foreground text-sm mt-1">
              Redirecting to beautiful resume view with PDF download option...
            </p>
          </div>,
        );
      ...
      // Grep command - search in project files
      if (command === "grep") {
        ...
        if (!pattern) {
          addCommand(
            input,
            <div className="font-mono">
              <p className="text-terminal-error">Usage: grep &lt;pattern&gt;</p>
              <p className="text-muted-foreground text-sm mt-1">
                Example: grep useState
              </p>
            </div>,
          );
          return;
        ...
        addCommand(
          input,
          <GrepOutput pattern={pattern} projectId={selectedProject?.id} />,
        );
        return;
      }

      // Game command - navigate to games page
      if (
        trimmedInput === "game" ||
        trimmedInput === "/game" ||
        trimmedInput === "games"
        ...
        addCommand(
          input,
          <div className="font-mono">
            <p className="text-terminal-success">🎮 Opening Game Zone...</p>
            <p className="text-muted-foreground text-sm mt-1">
              Redirecting to games page with Chess and Ludo...
            </p>
          </div>,
        );
        setTimeout(() => {
      ...
      // DevLab command
      if (trimmedInput === "devlab" || trimmedInput === "/devlab") {
        addCommand(
          input,
          <div className="font-mono">
            <p className="text-terminal-success">
              🧪 Opening DevLab Engineering Platform...
            </p>
            <p className="text-muted-foreground text-sm mt-1">
              Debug Playground • React Internals • Performance Lab • System
              Design
            </p>
          </div>,
        );
        setTimeout(() => {
          window.location.href = "/devlab";
        }, 500);
        return;
      }
